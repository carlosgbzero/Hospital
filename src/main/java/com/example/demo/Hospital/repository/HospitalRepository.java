...
public class HospitalRepository implements CRUD<Hospital> {
    ...
    public void create(Hospital entity) {
        try (Connection conn = dataSource.getConnection()) {
            String query = "INSERT INTO HOSPITAL (hospitalcod, nombre) VALUES (?, ?)";
    ...
    public Hospital find(int id) {
        ...
        try (Connection conn = dataSource.getConnection()) {
            String query = "SELECT * FROM HOSPITAL WHERE hospitalcod = ?";
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.setInt(1, id);
            ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
    ...
    public void update(Hospital entity, int id) {
        try (Connection conn = dataSource.getConnection()) {
            String query = "UPDATE HOSPITAL SET nombre = ? WHERE hospitalcod = ?";
            PreparedStatement stmt = conn.prepareStatement(query);
    ...
    public void delete(int id) {
        try (Connection conn = dataSource.getConnection()) {
            String query = "DELETE FROM HOSPITAL WHERE hospitalcod = ?";
            PreparedStatement stmt = conn.prepareStatement(query);
            stmt.setInt(1, id);
            stmt.executeUpdate();
        } catch (Exception e) {
